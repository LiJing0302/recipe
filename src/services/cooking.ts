...
export const clearCookingMemory = () => recordMemory.splice(0, recordMemory.length)
...
export const completeCooking = async (input: Omit<CookingRecord, 'id'>) => { const record = await createCookingRecordRemote(input); recordMemory.unshift(record); return record }
